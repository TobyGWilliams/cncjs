...
class Visualizer extends Component {
  ...
  updateStateFromStore = data => {
      ...
      this.machineTravel.updateTravels(
          convertMachineStoretoAxisLimits(currentMachine)
      );
      this.updateScene();
  };
  ...
  createCoordinateSystem(units) {
      const axisLength =
      units === IMPERIAL_UNITS ? IMPERIAL_AXIS_LENGTH : METRIC_AXIS_LENGTH;
      const gridCount =
      units === IMPERIAL_UNITS ? IMPERIAL_GRID_COUNT : METRIC_GRID_COUNT;
      const gridSpacing =
  ...
  load(name, gcode, callback) {
      ...
      // Display the name of the G-code file
          ...
          const posx = center.x;
          const posy =
        Math.floor(bbox.min.y / gridLength) * gridLength - gridLength / 2;
          const posz =
        Math.ceil(bbox.max.z / gridLength) * gridLength + gridLength / 2;
